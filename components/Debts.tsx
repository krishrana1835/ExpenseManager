...
const DebtItem: React.FC<DebtItemProps> = ({
  ...
  <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm flex justify-between items-center">
    <div onClick={() => onSelect(friend)} className="flex-1 cursor-pointer">
      ...
      <p
        className={`text-sm ${friend.balance > 0 ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}`}
      >
    ...
    <div className="flex items-center gap-4">
      <p
        className={`font-bold text-lg ${friend.balance > 0 ? "text-emerald-500" : "text-rose-500"}`}
      >
...
const Debts = ({
  ...
  const [settlementFriend, setSettlementFriend] =
    useState<FriendBalance | null>(null);
  ...
  const { peopleUserOwes, peopleWhoOweUser } = useMemo(() => {
      ...
      if (expense.paidBy === user.email) {
        ...
        if (userSplit) {
          balances[expense.paidBy] =
            (balances[expense.paidBy] || 0) - userSplit.amount;
        }
      }
    });
    ...
    const peopleUserOwes = allBalances
      .filter((f) => f.balance < 0)
      .sort((a, b) => a.balance - b.balance);
    const peopleWhoOweUser = allBalances
      .filter((f) => f.balance > 0)
  ...
  const handleSettleDebt = async (settlementAmount: number) => {
    ...
    const settlementExpenseData = {
      ...
      reason:
        mode === "pay" ? `Settlement to ${name}` : `Settlement from ${name}`,
      ...
      splits:
        mode === "pay"
          ? [
              { email: email, amount: settlementAmount },
              { email: user.email, amount: 0 },
            ]
          : [
              { email: user.email, amount: settlementAmount },
  ...
  const selectedFriendTransactions = useMemo(() => {
    ...
    const parseDate = (d: any): Date => {
      ...
      return new Date(0); // fallback (oldest)
    };

    return expenses
      .filter(
        (exp) =>
          exp.participants.includes(selectedFriend.email) &&
          exp.participants.includes(user.email) &&
          ...
          <div>
            ...
            <div className="flex justify-between items-center">
              <div>
                ...
                <p
                  className={`font-semibold ${selectedFriend.balance > 0 ? "text-emerald-500" : "text-rose-500"}`}
                >
                  {selectedFriend.balance > 0
                    ? `Owes you ${new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR" }).format(selectedFriend.balance)}`
                    : `You owe ${new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR" }).format(Math.abs(selectedFriend.balance))}`}
                </p>
              </div>
            </div>
            ...
            <div className="mt-4 bg-white dark:bg-gray-800 rounded-xl shadow-md">
              <div className="divide-y divide-gray-200 dark:divide-gray-700">
                {selectedFriendTransactions.length > 0 ? (
                  selectedFriendTransactions.map((exp) => (
                    <div key={exp.id} className="p-4">
                      ...
                      <p className="text-sm mt-2 text-gray-600 dark:text-gray-400">
                        {(() => {
                          const userShare =
                            exp.splits.find((s) => s.email === user.email)
                              ?.amount || 0;
                          const friendShare =
                            exp.splits.find(
                              (s) => s.email === selectedFriend.email
                            )?.amount || 0;
                  ...
                  <p className="p-8 text-center text-gray-500 dark:text-gray-400">
                    No transactions with {selectedFriend.name}.
                  </p>
                )}
              </div>
            </div>
              ...
              {debtView === "owe" && (
                <>
                  {peopleUserOwes.length > 0 ? (
                    peopleUserOwes.map((friend) => (
                      <DebtItem
                        key={friend.email}
                        friend={friend}
                        onSelect={setSelectedFriend}
                        onSettle={handleOpenSettleModal}
                        settlingEmail={settling}
                      />
                    ))
                  ) : (
