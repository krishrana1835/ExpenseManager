...
const SettleDebtModal: React.FC<SettleDebtModalProps> = ({ isOpen, onClose, friend, mode, onSettle }) => {
    ...
    const title = mode === 'pay' ? `Pay ${friend.name}` : `Receive from ${friend.name}`;
    const promptText = mode === 'pay' ? `You owe a total of ₹${totalDebt.toFixed(2)}` : `${friend.name} owes you a total of ₹${totalDebt.toFixed(2)}`;
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (settlementAmount > totalDebt + 0.001) { // Add tolerance for float issues
            setError(`Amount cannot exceed ₹${totalDebt.toFixed(2)}`);
            return;
        }
        setError('');
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-sm">
                <div className="p-6">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">{promptText}</p>
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Amount to Settle (₹)</label>
                            <input
                                type="text"
                                value={amount}
                        ...
                        <div className="flex justify-end space-x-3 pt-2">
                            ...
                            <button type="submit" disabled={!!error || !amount} className="px-4 py-2 text-sm font-medium light:text-black dark:text-white bg-primary rounded-md outline-1 
                            hover:bg-primary-600 disabled:bg-gray-400 disabled: text-white">
