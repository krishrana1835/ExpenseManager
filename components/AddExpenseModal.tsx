        ...
        <div className="p-6 sm:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <ul className="absolute z-10 w-full mt-1 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg max-h-48 overflow-y-auto divide-y divide-gray-200 dark:divide-gray-600 ring-1 ring-black ring-opacity-5 focus:outline-none">
                  {suggestions.map(s => (
                    ...
                    >
                      ...
                      <span className="text-gray-500 dark:text-gray-400 text-xs">{s.email}</span>
                    </li>
                  ))}
                </ul>
              )}
            ...
            <div className="space-y-2">
              {participants.map(p => (
                <div key={p.email} className="flex justify-between items-center bg-gray-100 dark:bg-gray-700/50 p-3 rounded-lg border border-gray-200 dark:border-gray-600 transition-colors hover:bg-gray-200 dark:hover:bg-gray-700">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="text-sm font-semibold text-gray-900 dark:text-white">
                        {p.name}
                        {p.email === user.email && <span className="text-xs font-normal text-gray-500 dark:text-gray-400 ml-1">(You)</span>}
                      </span>
                    </div>
                  </div>
                  {p.email !== user.email && (
            ...
            {participants.length > 0 && (
              <div className="space-y-4 pt-4 border-t border-gray-200 dark:border-gray-600">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center p-1 bg-gray-200 dark:bg-gray-700 rounded-full shadow-inner">
                    <button
                      ...
                      className={`px-4 py-2 text-sm font-medium rounded-full transition-all ${splitMode === 'equal' ? 'bg-white dark:bg-gray-600 shadow-md text-primary' : 'text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white'
                    ...
                    <button
                      ...
                      className={`px-4 py-2 text-sm font-medium rounded-full transition-all ${splitMode === 'manual' ? 'bg-white dark:bg-gray-600 shadow-md text-primary' : 'text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white'
                        } focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 dark:focus:ring-offset-gray-800`}
                    >
                      Manual
                    </button>
                ...
                <div className="space-y-3">
                  {splits.map(s => (
                    <div key={s.email} className="flex justify-between items-center gap-4 p-3 bg-gray-50 dark:bg-gray-700/30 rounded-lg border border-gray-200 dark:border-gray-600">
                      <div className="flex items-center gap-3 truncate">
                        <div className="h-8 w-8 rounded-full bg-gray-200 dark:bg-gray-600 flex items-center justify-center text-gray-600 dark:text-gray-300 font-semibold text-sm">
                          {participants.find(p => p.email === s.email)?.name.charAt(0).toUpperCase()}
                        </div>
                      ...
                      {splitMode === 'equal' ? (
                        ...
                        <div className="flex items-center relative rounded-md shadow-sm">
                          ...
                          <input
                            ...
                            onChange={(e) => handleManualSplitChange(s.email, e.target.value)}
                ...
                {splitMode === 'manual' && (
                  <div className={`text-right text-sm font-medium mt-2 flex justify-end items-center gap-2 ${isManualSplitValid ? 'text-gray-500 dark:text-gray-400' : 'text-red-500'
                    }`}>
                    <span>Total:</span>
                    <span className={isManualSplitValid ? 'text-gray-900 dark:text-white font-bold' : ''}>
                      ₹{manualTotal.toFixed(2)}
                    </span>
                    <span className="text-gray-400 dark:text-gray-500">/</span>
                    <span>₹{(parseFloat(amount) || 0).toFixed(2)}</span>
                    {isManualSplitValid && <CheckCircleIcon className="w-5 h-5 text-green-500" />}
                  </div>
                )}
              </div>
            )}
            ...
            <div className="flex justify-end space-x-4 pt-4 border-t border-gray-200 dark:border-gray-600">
              ...
              <button
                type="submit"
                disabled={loading || (splitMode === 'manual' && !isManualSplitValid)}
                className="flex items-center gap-2 px-6 py-3 text-sm font-medium text-white bg-primary rounded-lg shadow-sm hover:bg-primary-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors dark:focus:ring-offset-gray-800"
              >
                {loading ? (
                  <>
